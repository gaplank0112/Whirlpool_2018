...
def get_location_obj(object_name):
    # debug_obj.trace(med," Find object to match name string " + object_name.upper())
    try:
        return sim_server.Site(object_name.upper())
    except:
        pass

    try:
        return sim_server.Customer(object_name.upper())
    except:
        pass
    err_msg = "Could not find a site or customer matching name %s in Utilities module" % object_name.upper()
    debug_obj.logerror(low,err_msg)
    return 0
...
def split_promise_date_ship_condition(order_number):
    # The order number is a concatenation of promise date and ship condition in format dd/mm/yyyy_ShipCondition
    # Ship condition is a 2-digit alpha code at the end of the order number
    if '_' in order_number:
        promise_date, ship_condition = order_number.split('_')
    else:
        promise_date = order_number
        ship_condition = ''

    return promise_date, ship_condition
...
def check_daylight_savings(input_datetime):  # TODO: create better lookups for location based DST & flexible start/ends
